...
def test_setting_a_category_from_the_ui_sticks(client):
    ...
    response = client.post(
        "/api/overrides", json={"name_key": "PRYLBURK XYZ", "category": "Städ & hushåll"}
    )
    ...
    assert [i["category"] for i in items if i["name"] == "PRYLBURK XYZ"] == ["Städ & hushåll"]


def test_removing_an_override_restores_the_rules(client):
    client.post("/api/overrides", json={"name_key": "prylburk xyz", "category": "Skafferi"})
...
def test_an_edited_categories_file_is_not_overwritten(tmp_path, monkeypatch):
    monkeypatch.setenv("ICAKORT_DATA_DIR", str(tmp_path))
    path = tmp_path / "categories.yaml"
    path.write_text("fallback: Eget\ncategories: []\n", encoding="utf-8")

    config.ensure_categories_file()
    assert path.read_text(encoding="utf-8") == "fallback: Eget\ncategories: []\n"
    assert categorize.load_ruleset().fallback == "Eget"


def test_non_ascii_credentials_are_rejected_not_crashed(client, monkeypatch):
    """secrets.compare_digest kastar på icke-ASCII-str -- det får inte bli 500."""
    monkeypatch.setenv("ICAKORT_PASSWORD", "hemligt")
    assert client.get("/", auth=("någon", "hemligt")).status_code == 401
    assert client.get("/", auth=("icakort", "lösenord")).status_code == 401
